use agentic_planning::{
    ...
    GoalFilter, GoalId, GoalRelationship, GoalStatus, PathId, PlanningEngine, Priority, Promise,
    ReincarnationUpdates, Stakeholder, StakeholderId, Timestamp,
};
...
#[test]
fn edge_case_goal_linking() {
    let mut engine = PlanningEngine::in_memory();

    let a = engine
        .create_goal(CreateGoalRequest {
            title: "A".to_string(),
            intention: "Alpha".to_string(),
            ..Default::default()
        })
        .unwrap();
    let b = engine
        .create_goal(CreateGoalRequest {
            title: "B".to_string(),
            intention: "Beta".to_string(),
            ..Default::default()
        })
        .unwrap();

    engine
        .link_goals(GoalRelationship::Alliance {
            goals: (a.id, b.id),
            synergy: 0.8,
        })
        .unwrap();

    assert_eq!(engine.get_goal(a.id).unwrap().relationships.len(), 1);
    assert_eq!(engine.get_goal(b.id).unwrap().relationships.len(), 1);
}

#[test]
fn edge_case_due_soon_commitments() {
    let mut engine = PlanningEngine::in_memory();
    let c = engine
        .create_commitment(CreateCommitmentRequest {
            promise: Promise {
                description: "Edge commitment".to_string(),
                ..Default::default()
            },
            stakeholder: Stakeholder {
                id: StakeholderId(Uuid::new_v4()),
                name: "Edge Stakeholder".to_string(),
                ..Default::default()
            },
            due: Some(Timestamp::days_from_now(2.0)),
            ..Default::default()
        })
        .unwrap();

    let due = engine.get_due_soon(7.0);
    assert!(due.iter().any(|x| x.id == c.id));
}

#[test]
fn edge_case_search_and_filter() {
    let mut engine = PlanningEngine::in_memory();
    let goal = engine
        .create_goal(CreateGoalRequest {
            title: "Risk Model".to_string(),
            intention: "Build explainable risk model".to_string(),
            tags: Some(vec!["ai".to_string(), "risk".to_string()]),
            ..Default::default()
        })
        .unwrap();
    ...
    let found = engine.search_goals("explainable");
    assert_eq!(found.len(), 1);

    let filtered = engine.list_goals(GoalFilter {
...
#[test]
fn stress_repeated_persistence_writes() {
    let temp_dir = tempfile::tempdir().unwrap();
    let path = temp_dir.path().join("stress.aplan");
    let mut engine = PlanningEngine::open(&path).unwrap();

    for i in 0..50 {
        let _ = engine.create_goal(CreateGoalRequest {
            title: format!("Persistent {i}"),
            intention: "Write-heavy".to_string(),
            ..Default::default()
        });
        engine.save().unwrap();
    }
    ...
    let engine = PlanningEngine::open(&path).unwrap();
    assert_eq!(engine.list_goals(GoalFilter::default()).len(), 50);
}

#[test]
fn stress_relationship_graph_reads() {
    let mut engine = PlanningEngine::in_memory();
    ...
    for i in 0..80 {
        let goal = engine
            .create_goal(CreateGoalRequest {
                title: format!("Graph Goal {i}"),
                intention: "graph stress".to_string(),
                ..Default::default()
            })
            .unwrap();
        ids.push(goal.id);
    ...
    for pair in ids.windows(2) {
        engine
            .link_goals(GoalRelationship::Dependency {
                dependent: pair[1],
                on: pair[0],
                strength: 0.5,
            })
...
#[test]
fn scenario_17_metamorphosis_lifecycle() {
    let mut engine = PlanningEngine::in_memory();
    let goal = engine
        .create_goal(CreateGoalRequest {
            title: "Transformable Goal".to_string(),
            intention: "Validate metamorphosis".to_string(),
            ..Default::default()
        })
        .unwrap();
    engine.activate_goal(goal.id).unwrap();
    ...
    let blocker = Blocker {
        ...
        blocker_type: BlockerType::Unknown {
            signals: vec!["metamorphosis-test".to_string()],
        },
        description: "Hard blocker requiring directional change".to_string(),
        severity: 0.9,
        identified_at: Timestamp::now(),
        resolved_at: None,
        resolution: None,
    };
    engine.block_goal(goal.id, blocker).unwrap();

    let signal = engine.detect_metamorphosis(goal.id).unwrap();
    assert!(signal.should_transform);

    let updated = engine
        .approve_metamorphosis(
            goal.id,
            "Pivot Stage".to_string(),
            "Move to alternate execution path".to_string(),
            signal.recommended_change.clone(),
        )
        .unwrap();
    assert!(updated.metamorphosis.is_some());

    let history = engine.metamorphosis_history(goal.id).unwrap();
    assert_eq!(history.len(), 1);
    assert_eq!(history[0].title, "Pivot Stage");

    let current = engine.metamorphosis_stage(goal.id).unwrap().unwrap();
    assert_eq!(current.title, "Pivot Stage");

    let prediction = engine.predict_metamorphosis(goal.id).unwrap();
    assert!((0.0..=1.0).contains(&prediction.confidence));
}

#[test]
fn scenario_18_merge_from_reports() {
    let mut source = PlanningEngine::in_memory();
    let mut target = PlanningEngine::in_memory();

    let source_goal = source
        .create_goal(CreateGoalRequest {
            title: "Source Goal".to_string(),
            intention: "source side".to_string(),
            ..Default::default()
        })
        .unwrap();
    source.activate_goal(source_goal.id).unwrap();
    source.dream_goal(source_goal.id).unwrap();
    source
        .create_decision(CreateDecisionRequest {
            question: "Source Decision".to_string(),
            ..Default::default()
        })
        .unwrap();
    source
        .create_commitment(CreateCommitmentRequest {
            promise: Promise {
                description: "Source Promise".to_string(),
                ..Default::default()
            },
            stakeholder: Stakeholder {
                id: StakeholderId(Uuid::new_v4()),
                name: "Source Stakeholder".to_string(),
                ..Default::default()
            },
            ..Default::default()
        })
        .unwrap();
    source
        .create_federation(source_goal.id, "agent-source".to_string(), None)
        .unwrap();

    target
        .create_goal(CreateGoalRequest {
            title: "Target Goal".to_string(),
            intention: "target side".to_string(),
            ..Default::default()
        })
        .unwrap();

    let report = target.merge_from(&source);
    assert!(report.goals_merged >= 1);
    assert!(report.decisions_merged >= 1);
    assert!(report.commitments_merged >= 1);
    assert!(report.dreams_merged >= 1);
    assert!(report.federations_merged >= 1);

    let second = target.merge_from(&source);
    assert_eq!(second.goals_merged, 0);
    assert_eq!(second.decisions_merged, 0);
    assert_eq!(second.commitments_merged, 0);
    assert_eq!(second.dreams_merged, 0);
    assert_eq!(second.federations_merged, 0);
}
